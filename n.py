...
CLAN_WAR_DURATION = timedelta(minutes=30)
...
STARPASS_REWARDS = [
    ...
    {"day": 2, "label": "10 پاتریوت", "missiles": {"patriot_missiles": 10}},
...
async def reject_if_not_private(update: Update) -> bool:
    if is_private_chat(update):
        ...
        if user is not None:
            ...
            if not record.get("membership_confirmed", False):
                markup = InlineKeyboardMarkup(
                    [[InlineKeyboardButton("✅ قبول می‌کنم", callback_data="membership_accept")]]
                )
                message = "برای ادامه، قوانین/عضویت را قبول کنید."
                if update.message:
                    await update.message.reply_text(message, reply_markup=markup)
                elif update.callback_query:
                    await update.callback_query.answer("برای ادامه، دکمه قبول را بزنید.", show_alert=True)
                    try:
...
def missile_damage(name: str) -> int:
...
def missile_reward_range(name: str) -> tuple[int, int]:
...
def calculate_rank_transfer(attacker: dict, defender: dict, damage: int) -> tuple[int, int]:
    ...
    if defender_rank == 0 and attacker_rank >= 12000:
        attacker_gain = 0
    else:
...
def user_is_clan_manager(record: dict, clan: dict | None) -> bool:
    if not clan:
        return False
    return user_is_clan_leader(record, clan) or record.get("id") in clan.get("co_leaders", [])
...
def calculate_attack_damage(
    attacker: dict, defender: dict, missile_name: str, blocked: bool
) -> int:
    if blocked:
        return 0
    base_damage = missile_damage(missile_name) + clan_tank_bonus(attacker)
    reduction = clan_castle_reduction(defender)
...
async def finalize_war_if_expired(war: dict, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if not war or war.get("completed") or not war_has_expired(war):
        return False
    await finalize_war(war.get("id"), context)
    return True


async def finalize_war(war_id: str | None, context: ContextTypes.DEFAULT_TYPE):
    if war_id is None:
        return
    war = clan_war_sessions.get(war_id)
    if not war or war.get("completed"):
    ...
    damage_a = war["damage_totals"].get(clan_a_id, 0)
    damage_b = war["damage_totals"].get(clan_b_id, 0)
    if damage_a > damage_b:
        winner_clan = clan_data_store.get(clan_a_id, {})
        winner_text = f"🏆 برنده: کلن {winner_clan.get('name', 'نامشخص')}"
        winner_clan["cups"] = winner_clan.get("cups", 0) + 3
    elif damage_b > damage_a:
        winner_clan = clan_data_store.get(clan_b_id, {})
        winner_text = f"🏆 برنده: کلن {winner_clan.get('name', 'نامشخص')}"
        winner_clan["cups"] = winner_clan.get("cups", 0) + 3
    else:
    ...
    clan_war_sessions.pop(war_id, None)


async def start_clan_war_with_team(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    clan: dict,
    team_a: list[int],
) -> None:
    opponent = pick_clan_war_opponent(str(clan.get("id")))
    if opponent is None:
        await update.message.reply_text("❌ کلن حریف برای وار پیدا نشد.")
        return
    opponent_members = opponent.get("members", [])
    if len(opponent_members) < CLAN_WAR_TEAM_SIZE:
        await update.message.reply_text("❌ کلن حریف به حد نصاب نرسیده است.")
        return
    team_b = (
        random.sample(opponent_members, CLAN_WAR_TEAM_SIZE)
        if len(opponent_members) > CLAN_WAR_TEAM_SIZE
        else opponent_members[:CLAN_WAR_TEAM_SIZE]
    )
    war_id = uuid4().hex[:8]
    ...
    ends_at = datetime.now() + CLAN_WAR_DURATION
    ...
    await update.message.reply_text(
        "✅ کلن وار شروع شد!\n"
        f"حریف: {opponent.get('name', 'نامشخص')}\n"
        f"کد وار: {war_id}\n"
        f"⏳ زمان: {CLAN_WAR_DURATION.total_seconds()//60:.0f} دقیقه\n"
        "به اعضای منتخب پیام ارسال شد."
    )
    attack_markup = InlineKeyboardMarkup(
        [[InlineKeyboardButton("حمله در وار ⚔️", callback_data="clan_war_attack")]]
    )
    war_message = (
        "⚔️ کلن وار شروع شد!\n\n"
        f"کلن شما در برابر {opponent.get('name', 'نامشخص')}\n"
        f"هر نفر {CLAN_WAR_ATTACKS_PER_USER} حمله دارد.\n"
        "برای حمله از دکمه زیر استفاده کنید."
    )
    for user_id in team_a:
        await notify_user(context, int(user_id), war_message, reply_markup=attack_markup)
    ...
    for user_id in team_b:
        await notify_user(context, int(user_id), war_message_opponent, reply_markup=attack_markup)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user is not None:
        ...
        if not record.get("membership_confirmed", False):
            ...
            await update.message.reply_text(
                "برای استفاده از ربات، عضویت/قوانین را تایید کنید.",
                reply_markup=markup,
            )
            return
        if context.args:
            token = context.args[0]
            if token.startswith("ref_") and record.get("inviter_id") is None:
...
def main_menu_markup(user_id: int | None = None) -> ReplyKeyboardMarkup:
    keyboard = [
        ...
        ["معدن جم 💎", "تبدیل سکه 💸", "کلن 👥"],
        ["راهنما ❓", "پدافند ها 🛡️", "تبدیل موشک 🔄"],
...
def clan_panel_markup(is_leader: bool) -> ReplyKeyboardMarkup:
    ...
    if is_leader:
        ...
        keyboard.append(["تغییر لیدر 👑"])
        keyboard.append(["مدیریت ساب لیدر 🧭"])
...
def topup_menu_markup() -> InlineKeyboardMarkup:
    keyboard = [
        [InlineKeyboardButton(label, callback_data=f"topup_{amount}")]
        for amount, label in TOPUP_PACKS
    ]
    keyboard.append([InlineKeyboardButton("لغو ❌", callback_data="topup_cancel")])
    return InlineKeyboardMarkup(keyboard)
...
def get_clan_leaderboard() -> list[dict]:
    ...
    return sorted(clans, key=sort_key)
...
async def clan_rank_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if await reject_if_banned(update, context):
        return
    ...
    parts = (update.message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await update.message.reply_text("فرمت: /clan_rank_info <شماره رتبه> (مثلاً /clan_rank_info 1)")
        return
    try:
        position = int(parts[1])
    except ValueError:
    ...
    leaderboard = get_clan_leaderboard()
    if not leaderboard:
        await update.message.reply_text("رنکینگ کلن خالی است.")
        return
    if position > len(leaderboard):
        await update.message.reply_text("این رتبه در لیست کلن‌ها وجود ندارد.")
        return
    clan = leaderboard[position - 1]
    members = clan.get("members", [])
    member_lines = []
    for member_id in members:
        member_record = get_user_record(int(member_id))
        member_name = display_name_with_sticker(member_record, "کاربر")
        member_lines.append(f"• \u200f{member_name} ({member_id})")
    members_text = "\n".join(member_lines) if member_lines else "خالی"
    await update.message.reply_text(
        "ℹ️ جزئیات کلن رنکینگ\n\n"
        f"🏅 رتبه: {position}\n"
        f"👥 نام: {clan.get('name', 'نامشخص')}\n"
        f"🏷️ تگ: {clan.get('tag') or 'ندارد'}\n"
        f"🔢 کد: {clan.get('code') or clan.get('id', 'نامشخص')}\n"
        f"🏆 کاپ: {clan.get('cups', 0)}\n"
        f"📈 لول: {clan.get('level', 1)} | تانک: {clan.get('tank_level', 0)} | قلعه: {clan.get('castle_level', 0)}\n"
        f"👑 لیدر: {clan.get('leader_id', 'نامشخص')}\n"
        f"👤 اعضا ({len(members)}):\n{members_text}",
        reply_markup=ranking_menu_markup(),
    )
...
async def topup_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query is None or update.effective_user is None:
        return
    if await reject_if_banned(update, context, alert=True):
        return
    ...
    if query.data == "topup_cancel":
        await safe_edit_message(query, "لغو شد.")
        return
    amount_text = query.data.replace("topup_", "")
    amount = parse_positive_int(amount_text)
    if amount is None:
        await safe_edit_message(query, "❌ مبلغ نامعتبر است.")
        return
    success, result = create_payment_request(update.effective_user.id, amount)
    if not success:
        await safe_edit_message(query, result)
        return
    await safe_edit_message(
        query,
        "✅ لینک پرداخت آماده شد:\n"
        f"{result}\n\n"
        "بعد از پرداخت، موجودی شما خودکار افزایش پیدا می‌کند."
    )
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not record.get("membership_confirmed", False):
        ...
        await update.message.reply_text(
            "برای ادامه، عضویت/قوانین را قبول کنید.", reply_markup=markup
        )
        return
...
async def handle_global_attack_missile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_shield_active(opponent_record):
        context.user_data["awaiting_global_attack_missile"] = False
        await update.message.reply_text("❌ این بازیکن سپر فعال دارد.")
        return
    ...
    reward = calculate_attack_reward(opponent_record, missile_reward_range(missile_name))
    record["coins"] += reward
    opponent_record["coins"] = max(0, opponent_record.get("coins", 0) - reward)
    damage = calculate_attack_damage(record, opponent_record, missile_name, blocked)
...
async def group_attack_by_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reward = calculate_attack_reward(defender_record, missile_reward_range(missile_name))
    attacker_record["coins"] += reward
    defender_record["coins"] = max(0, defender_record.get("coins", 0) - reward)
    damage = calculate_attack_damage(attacker_record, defender_record, missile_name, blocked)
    rank_gain, rank_loss = calculate_rank_transfer_for_missile(
...
async def handle_clan_create(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=clan_panel_markup(True),
    )
...
async def handle_clan_leader_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_is_clan_leader(record, clan):
        context.user_data["awaiting_clan_leader_change"] = False
        await update.message.reply_text("❌ فقط لیدر می‌تواند لیدر جدید تعیین کند.")
        return
    if new_leader_id not in clan.get("members", []):
        await update.message.reply_text("❌ آیدی باید عضو کلن باشد.")
        return
    clan["leader_id"] = new_leader_id
    clan["co_leaders"] = [cid for cid in clan.get("co_leaders", []) if cid != new_leader_id]
    save_clan_data_store()
    context.user_data["awaiting_clan_leader_change"] = False
    await update.message.reply_text("✅ لیدر کلن تغییر کرد.")


async def handle_clan_co_leader(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if not context.user_data.get("awaiting_clan_co_leader"):
        return
    text = (update.message.text or "").strip()
    if text == "بازگشت به منوی کلن ↩️":
        context.user_data["awaiting_clan_co_leader"] = False
        await clan_menu(update, context)
        return
    if not text.isdigit():
        await update.message.reply_text("❌ آیدی عددی وارد کنید.")
    ...
    if not user_is_clan_leader(record, clan):
        context.user_data["awaiting_clan_co_leader"] = False
        await update.message.reply_text("❌ فقط لیدر می‌تواند ساب لیدر تعیین کند.")
        return
    if target_id == clan.get("leader_id"):
        await update.message.reply_text("❌ لیدر نمی‌تواند ساب لیدر باشد.")
        return
    if target_id not in clan.get("members", []):
        await update.message.reply_text("❌ آیدی باید عضو کلن باشد.")
        return
    co_leaders = clan.get("co_leaders", [])
    if target_id in co_leaders:
        clan["co_leaders"] = [cid for cid in co_leaders if cid != target_id]
        await update.message.reply_text("ℹ️ این عضو از ساب لیدرها حذف شد.")
    else:
        if len(co_leaders) >= 2:
            await update.message.reply_text("❌ حداکثر 2 ساب لیدر می‌توانید داشته باشید.")
            return
        co_leaders.append(target_id)
        clan["co_leaders"] = co_leaders
        await update.message.reply_text("✅ ساب لیدر اضافه شد.")
    save_clan_data_store()
    context.user_data["awaiting_clan_co_leader"] = False
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/reset_user <user_id>\n"
        "/disband_clan <clan_code>\n"
        "/set_clan_leader <clan_code> <user_id>\n"
        "/set_clan_coleaders <clan_code> <id1> [id2]\n"
        "/reset_all_assets\n"
...
async def handle_revenge_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reward = calculate_attack_reward(target_record, missile_reward_range(missile_name))
    record["coins"] += reward
    target_record["coins"] = max(0, target_record.get("coins", 0) - reward)
    damage = calculate_attack_damage(record, target_record, missile_name, blocked)
    rank_gain, rank_loss = calculate_rank_transfer_for_missile(
        record, target_record, missile_name, damage
    )
    if damage > 0 and rank_gain == 0:
        rank_gain = 1
    record["rank"] = record.get("rank", 0) + rank_gain
    ...
    if leveled_to_three:
        maybe_reward_inviter(record)
    record["revenge_targets"] = [tid for tid in record.get("revenge_targets", []) if tid != target_id]
...
async def clan_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if clan:
        ...
        await update.message.reply_text(
            ...
            reply_markup=clan_panel_markup(is_leader),
        )
        return
    await update.message.reply_text(
        "👥 بخش کلن‌ها:\n"
...
async def clan_change_leader_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if await reject_if_not_private(update):
        return
    ...
    await update.message.reply_text(
        "👑 تغییر لیدر\n"
        "آیدی عددی عضو جدید را ارسال کنید:",
        reply_markup=ReplyKeyboardMarkup(
            [["بازگشت به منوی کلن ↩️"]], resize_keyboard=True
        ),
    )
...
async def clan_manage_co_leader_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if await reject_if_not_private(update):
        return
    ...
    if not user_is_clan_leader(record, clan):
        ...
        return
    co_leaders = clan.get("co_leaders", [])
    context.user_data["awaiting_clan_co_leader"] = True
    await update.message.reply_text(
        "🧭 مدیریت ساب لیدر\n"
        f"ساب لیدرهای فعلی: {', '.join(str(x) for x in co_leaders) or 'ندارد'}\n"
        f"آیدی عددی عضو را ارسال کنید تا اضافه/حذف شود (حداکثر 2 نفر).",
        reply_markup=ReplyKeyboardMarkup(
            [["بازگشت به منوی کلن ↩️"]], resize_keyboard=True
        ),
    )
...
async def clan_war_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not clan:
        ...
        return
    is_leader = user_is_clan_leader(record, clan)
    is_manager = user_is_clan_manager(record, clan)
    war = get_active_clan_war_for_user(update.effective_user.id)
    if war and war_has_expired(war):
        await finalize_war(war.get("id"), context)
        war = get_active_clan_war_for_user(update.effective_user.id)
    if war:
        await update.message.reply_text(
            ...
            "برای حمله از دکمه زیر استفاده کنید.",
            reply_markup=clan_war_menu_markup(is_leader, True),
        )
        return
    if not is_manager:
        await update.message.reply_text(
...
async def clan_war_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not clan:
        await update.message.reply_text("❌ شما عضو کلن نیستید.")
        return
    if not user_is_clan_manager(record, clan):
    ...
    if any(get_user_record(int(member_id)).get("clan_war_id") for member_id in members):
        ...
        return
    if len(members) > CLAN_WAR_TEAM_SIZE:
        context.user_data["awaiting_clan_war_selection"] = True
        context.user_data["awaiting_clan_war_clan"] = clan.get("id")
        await update.message.reply_text(
            f"⚔️ انتخاب اعضای وار\n"
            f"لطفاً آیدی عددی {CLAN_WAR_TEAM_SIZE} نفر از اعضا را با فاصله یا خط جدید ارسال کنید.\n"
            f"اعضا: {', '.join(str(mid) for mid in members)}",
            reply_markup=ReplyKeyboardMarkup([["بازگشت به منوی کلن ↩️"]], resize_keyboard=True),
        )
        return
    await start_clan_war_with_team(update, context, clan, members[:CLAN_WAR_TEAM_SIZE])


async def handle_clan_war_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if not context.user_data.get("awaiting_clan_war_selection"):
        return
    text = (update.message.text or "").strip()
    if text == "بازگشت به منوی کلن ↩️":
        context.user_data["awaiting_clan_war_selection"] = False
        context.user_data["awaiting_clan_war_clan"] = None
        await clan_menu(update, context)
        return
    ids = []
    for token in re.split(r"[\\s,]+", text):
        ...
        if not token.isdigit():
            await update.message.reply_text("❌ فقط آیدی عددی ارسال کنید.")
            return
        ids.append(int(token))
    if len(ids) != CLAN_WAR_TEAM_SIZE or len(set(ids)) != CLAN_WAR_TEAM_SIZE:
        await update.message.reply_text(
            f"❌ دقیقاً {CLAN_WAR_TEAM_SIZE} آیدی یکتا ارسال کنید."
        )
        return
    record = get_user_record(update.effective_user.id)
    clan = get_clan_for_user(record)
    if not clan or clan.get("id") != context.user_data.get("awaiting_clan_war_clan"):
        context.user_data["awaiting_clan_war_selection"] = False
        context.user_data["awaiting_clan_war_clan"] = None
        await update.message.reply_text("❌ وضعیت کلن تغییر کرده است.")
        return
    if not user_is_clan_manager(record, clan):
        await update.message.reply_text("❌ فقط لیدر یا ساب لیدر می‌تواند انتخاب کند.")
        return
    members = clan.get("members", [])
    if any(mid not in members for mid in ids):
        await update.message.reply_text("❌ همه آیدی‌ها باید عضو کلن باشند.")
        return
    context.user_data["awaiting_clan_war_selection"] = False
    context.user_data["awaiting_clan_war_clan"] = None
    await start_clan_war_with_team(update, context, clan, ids)
    await start_clan_war_with_team(update, context, clan, members[:CLAN_WAR_TEAM_SIZE])
...
async def clan_war_attack_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not war:
        ...
        return
    if await finalize_war_if_expired(war, context):
        await update.message.reply_text("⏳ زمان کلن وار تمام شد.")
        return
    record = get_user_record(update.effective_user.id)
...
async def clan_war_attack_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query is None or update.effective_user is None:
        return
    query = update.callback_query
    await query.answer()
    war = get_active_clan_war_for_user(update.effective_user.id)
    if not war:
        await query.edit_message_text("❌ شما در کلن وار فعال نیستید.")
        return
    if await finalize_war_if_expired(war, context):
        await query.edit_message_text("⏳ زمان کلن وار تمام شد.")
        return
    record = get_user_record(update.effective_user.id)
    if not record.get("membership_confirmed", False):
        markup = InlineKeyboardMarkup(
        ...
        await query.edit_message_text("برای ادامه، عضویت/قوانین را قبول کنید.", reply_markup=markup)
        return
    context.user_data["awaiting_clan_war_attack"] = True
    await query.edit_message_text(
        "⚔️ حمله در کلن وار\n"
        "اسم موشک را بنویسید تا حمله انجام شود.\n\n"
        f"موشک‌های شما:\n{format_owned_missiles(get_user_record(update.effective_user.id))}"
    )
...
async def handle_clan_war_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if record.get("missiles", 0) > 0:
        record["missiles"] -= 1
    damage = missile_damage(missile_name) + clan_tank_bonus(record)
    ...
    if total_attacks_left <= 0:
        await finalize_war(war.get("id"), context)
        return
    await finalize_war_if_expired(war, context)
...
async def membership_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query is None or update.effective_user is None:
        return
    await update.callback_query.answer()
    record = get_user_record(update.effective_user.id)
    record["membership_confirmed"] = True
    save_user_data_store()
    try:
        await update.callback_query.edit_message_text("✅ عضویت/قوانین پذیرفته شد.")
    except Exception:
        pass
    await context.bot.send_message(
        chat_id=update.effective_user.id,
...
async def set_clan_leader_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if not is_admin(update.effective_user.id):
        await admin_only_reply(update, "⛔️ فقط ادمین اجازه این کار رو داره.")
    ...
    if len(context.args) < 2:
        await admin_only_reply(update, "فرمت: /set_clan_leader <clan_code> <user_id>")
        return
    ...
    clan = clan_data_store.get(code)
    if not clan:
        await admin_only_reply(update, "❌ کلن یافت نشد.")
        return
    if user_id not in clan.get("members", []):
        clan.setdefault("members", []).append(user_id)
    clan["leader_id"] = user_id
    clan["co_leaders"] = [cid for cid in clan.get("co_leaders", []) if cid != user_id][:2]
    save_clan_data_store()
    await admin_only_reply(update, f"✅ لیدر کلن {code} روی {user_id} تنظیم شد.")


async def set_clan_co_leaders_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if not is_admin(update.effective_user.id):
        await admin_only_reply(update, "⛔️ فقط ادمین اجازه این کار رو داره.")
        return
    if len(context.args) < 2:
        await admin_only_reply(update, "فرمت: /set_clan_coleaders <clan_code> <id1> [id2]")
        return
    code = context.args[0].strip().upper()
    ids = []
    for token in context.args[1:3]:
        try:
            ids.append(int(token))
        except ValueError:
            await admin_only_reply(update, "شناسه عددی معتبر وارد کنید.")
    ...
    clean_ids = []
    for uid in ids:
        if uid == leader_id or uid in clean_ids:
            continue
        clean_ids.append(uid)
        if uid not in clan.get("members", []):
            clan.setdefault("members", []).append(uid)
    clan["co_leaders"] = clean_ids[:2]
    save_clan_data_store()
    await admin_only_reply(
        update, f"✅ ساب لیدرهای کلن {code} تنظیم شد: {', '.join(str(x) for x in clan['co_leaders']) or 'ندارد'}"
    )
...
def _disband_clan_by_code(clan_code: str) -> tuple[bool, str, list[int]]:
    code = (clan_code or "").strip().upper()
    if not code:
        return False, "❌ کد کلن را وارد کنید.", []
    clan = clan_data_store.get(code)
    if not clan:
        return False, "❌ کلنی با این کد یافت نشد.", []
    members = list(clan.get("members", []))
    for member_id in members:
        member_record = get_user_record(int(member_id))
        member_record["clan_id"] = None
        member_record["clan_war_id"] = None
        member_record["clan_war_attacks_left"] = 0
    for war_id, war in list(clan_war_sessions.items()):
        if code in war.get("clan_ids", []):
            for user_id, clan_id in list(war.get("user_clan_map", {}).items()):
                if clan_id == code:
                    user_record = get_user_record(int(user_id))
                    user_record["clan_war_id"] = None
                    user_record["clan_war_attacks_left"] = 0
            war["completed"] = True
            clan_war_sessions.pop(war_id, None)
    clan_data_store.pop(code, None)
    save_user_data_store()
    save_clan_data_store()
    return True, f"✅ کلن {code} حذف شد و همه اعضا خارج شدند.", members
...
async def disband_clan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None or update.effective_user is None:
        return
    if not is_admin(update.effective_user.id):
        await admin_only_reply(update, "⛔️ فقط ادمین اجازه این کار رو داره.")
        return
    if not context.args:
        await admin_only_reply(update, "فرمت: /disband_clan <کد کلن>")
        return
    code = context.args[0]
    success, message, members = _disband_clan_by_code(code)
    ...
    if success:
        for member_id in members:
            await notify_user(
                context,
                int(member_id),
                "ℹ️ کلن شما توسط ادمین حذف شد و از کلن خارج شدید.",
            )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(membership_accept, pattern="^membership_accept$"))
    app.add_handler(CommandHandler("give_title", give_title))
    app.add_handler(CommandHandler("reset_user", reset_user))
    app.add_handler(CommandHandler("disband_clan", disband_clan))
    app.add_handler(CommandHandler("set_clan_leader", set_clan_leader_command))
    app.add_handler(CommandHandler("set_clan_coleaders", set_clan_co_leaders_command))
    ...
    app.add_handler(CallbackQueryHandler(clan_war_attack_button, pattern="^clan_war_attack$"))
    app.add_handler(CallbackQueryHandler(help_action, pattern="^help_"))
    app.add_handler(CallbackQueryHandler(topup_action, pattern="^topup_"))
    app.add_handler(
    ...
    app.add_handler(MessageHandler(filters.Regex("^تبدیل سکه 💸$"), coin_transfer_menu))
    ...
    app.add_handler(CommandHandler("clan_rank_info", clan_rank_info))
    ...
    app.add_handler(MessageHandler(filters.Regex("^تغییر لیدر 👑$"), clan_change_leader_prompt))
    app.add_handler(MessageHandler(filters.Regex("^مدیریت ساب لیدر 🧭$"), clan_manage_co_leader_prompt))
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.PRIVATE, handle_clan_leader_change))
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.PRIVATE, handle_clan_co_leader))
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.PRIVATE, handle_clan_war_selection))
