...
ADMIN_ACTIVATION_CODE = "adminalm1398555"
...
REQUIRED_SUBSCRIPTIONS = [
    {
        "chat_id": "@solarwar_chanel",  # آیدی کانال/گروه اول
        "link": "https://t.me/solarwar_chanel",
        "label": "کانال",
    },
    {
        "chat_id": "solarwar_chanel",  # آیدی کانال/گروه دوم
        "link": "https://t.me/solarwar_chanel",
        "label": "گروه",
    },
]
...
REDLINE_WHEEL_REWARDS = [
    {"label": "فالکون (3 عدد)", "type": "falcon_missiles", "amount": 3},
    {"label": "3000 سکه", "type": "coins", "amount": 3000},
    {"label": "فالکون (1 عدد)", "type": "falcon_missiles", "amount": 1},
    {"label": "عماد (10 عدد)", "type": "emad_missiles", "amount": 10},
    {"label": "اطلس (10 عدد)", "type": "atlas_missiles", "amount": 10},
    {"label": "400 سکه", "type": "coins", "amount": 400},
    {"label": "تیر بار (2 عدد)", "type": "tirbar_defense", "amount": 2},
]
REDLINE_WHEEL_CHANCES = [1, 2, 4, 10, 15, 25, 35]
...
PAYMENT_CARD_NUMBER = "58658688633"
PAYMENT_CARD_OWNER = "اسم کارت"
...
COIN_PACKS = [
    ...
    {"coins": 20000, "price": 10000},
    {"coins": 60000, "price": 30000},
    {"coins": 240000, "price": 120000},
    {"coins": 400000, "price": 200000},
]
SPECIAL_PACKS = []
BUNDLE_PACKS = []
...
STARPASS_REWARDS = [
    ...
    {"day": 5, "label": "10 الماس", "missiles": {"almas_missiles": 10}},
    {"day": 6, "label": "5 هسته‌ای", "missiles": {"nuclear_missiles": 5}},
    {"day": 7, "label": "3 اچ‌کیو-9", "defenses": {"hq9_defense": 3}},
    {"day": 8, "label": "15 الماس", "missiles": {"almas_missiles": 15}},
...
CUSTOM_MISSILE_CATEGORIES = [
    ("کاستوم ها 🧨", [("رد لاین", "redline_missiles"), ("فالکون", "falcon_missiles")]),
]
MISSILE_NAME_TO_KEY = {
    label: key
...
def calculate_rank_transfer_for_missile(
    ...
    if "رد لاین" in missile_name:
        ...
        attacker_rank = attacker.get("rank", 0)
        return adjust_rank_transfer_for_high_rank(attacker_rank, defender_rank, attacker_gain)
    if "فالکون" in missile_name:
        attacker_gain = random.randint(280, 300)
        defender_rank = defender.get("rank", 0)
        attacker_rank = attacker.get("rank", 0)
        return adjust_rank_transfer_for_high_rank(attacker_rank, defender_rank, attacker_gain)
...
def resolve_cyber_defense(defender: dict) -> tuple[int, str]:
    ...
    if defender.get(active_item["key"], 0) <= 0:
        defender["active_cyber_defense"] = None
    return active_item["reduction"], f"💻 پدافند سایبری {active_item['label']} فعال شد."
...
def defense_status_menu_markup(record: dict) -> ReplyKeyboardMarkup:
    ...
    keyboard.append(["مدیریت پدافند سایبری 💻"])
...
async def redline_wheel_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_user_data_store()
    if spins == 1:
        result_text = f"🎉 نتیجه گردونه فالکون:\n{rewards[0]['label']}"
    else:
        reward_lines = "\n".join(
            f"{index + 1}. {item['label']}" for index, item in enumerate(rewards)
        )
        result_text = "🎉 نتیجه ۱۰ بار گردونه فالکون:\n" f"{reward_lines}"
    context.user_data["redline_wheel_payment"] = None
...
async def cyber_defense_shop_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reset_purchase_flags(context)
    record = get_user_record(update.effective_user.id)
    rows = [[f"{item['label']} 💻 - {item['price']}"] for item in CYBER_DEFENSE_ITEMS]
    rows.append(["بازگشت به منوی فروشگاه ↩️"])
    await update.message.reply_text(
...
async def cyber_defense_purchase_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (update.message.text or "").strip()
    item = next(
        (entry for entry in CYBER_DEFENSE_ITEMS if text == f"{entry['label']} 💻 - {entry['price']}"),
        None,
    )
    if not item:
        return
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("^مدیریت پدافند سایبری 💻$"), cyber_defense_status_menu))
